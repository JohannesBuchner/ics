...
public class XmppCommons {
	...
	public static XMPPConnection login(String xmppid, String pw, String host,
		...
		String hostname = (host != null && host.length() > 0) ? host : xmppid
		...
		try {
			connection = XmppCommons.doLogin(usernameWithDomain, pw, hostname,
					port, resource);
		} catch (XMPPException e) {
			log.trace("Received XMPPException while trying to log in with full hostname: "
					+ e.getMessage());
		}
		...
		if (connection == null) {
			...
			connection = XmppCommons.doLogin(username, pw, hostname, port,
					resource);
		}
		return connection;
	}
	 ...
	 *             general XMPP error
	 */
	private static XMPPConnection doLogin(String username, String pw,
			String hostname, long port, String resource) throws IOException,
			XMPPException {

		log.debug("connecting to host: '" + hostname + "'" + " with port "
				+ port + " (port autosearch if 0)");
	...
	public static boolean isLoggedIn(XMPPConnection connection) {
		...
		log.trace(connection.getUser() + " - auth: "
